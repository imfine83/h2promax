      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-black text-white mt-4" style={{ fontFamily: 'Orbitron' }}>
            {c.title1} <span className="text-[#39ff14]">{c.title2}</span>
          </h2>
          <p className="mt-4 text-white/30 max-w-2xl mx-auto text-sm">{c.subtitle}</p>
        </div>
          ...
          {c.products.map((product) => (
            ...
            >
              ...
              <div className="relative h-52 bg-gradient-to-b from-[#111128] to-[#0a0a0f] flex items-center justify-center p-6 overflow-hidden">
                <img src={product.image} alt={`${product.series} ${product.model}`}
              ...
              <div className="p-5">
                ...
                <div className="text-2xl font-black text-white mb-5 font-mono-cyber">{product.price}</div>

                <div className="space-y-2">
                  <button onClick={() => openModal(product, 'install')}
                    ...
                    <ShoppingCart size={14} /> {c.orderBtn}
                  </button>
                  <button onClick={() => openModal(product, 'installment')}
                    ...
                    <CreditCard size={14} /> {c.installmentBtn}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-[#0d0d1a] border border-[#39ff14]/20 p-8 max-w-md w-full mx-4"
            ...
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-white" style={{ fontFamily: 'Orbitron', fontSize: '13px' }}>
                {modalType === 'install' ? c.modalOrder : c.modalInstallment}
              </h3>
              <button onClick={() => setShowModal(false)} className="text-white/30 hover:text-[#ff2d95] text-2xl transition-colors">&times;</button>
            </div>
            <div className="border border-[#39ff14]/15 p-4 mb-6 bg-[#39ff14]/5">
              <p className="font-bold text-white text-sm" style={{ fontFamily: 'Orbitron' }}>{selectedProduct.series} {selectedProduct.model}</p>
